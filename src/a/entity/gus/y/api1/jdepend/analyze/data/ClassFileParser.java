package a.entity.gus.y.api1.jdepend.analyze.data;
...
public class ClassFileParser {
	...
	private class Constant {
		...
		public Constant() throws Exception {
			...
			case (CONSTANT_NAMEANDTYPE):
				init(_unsigned(), _unsigned());
				break;
			case (CONSTANT_INTEGER):
				init(new Integer(_int()));
				break;
			case (CONSTANT_FLOAT):
				init(new Float(_float()));
				break;
			case (CONSTANT_LONG):
				init(new Long(_long()));
				break;
			case (CONSTANT_DOUBLE):
				init(new Double(_double()));
				break;
			case (CONSTANT_UTF8):
				init(_UTF());
				break;
			case (CONSTANT_METHOD_HANDLE):
				init(_byte(), _unsigned());
				break;
			...
			default:
				throw new Exception("Unknown constant: " + tag);
			}
		}
		...
		public String toString() {
			...
			s.append("tag: " + tag);
			if (nameIndex > -1)
				s.append(" nameIndex: " + nameIndex);
			if (typeIndex > -1)
				s.append(" typeIndex: " + typeIndex);
			if (value != null)
	...
	private FieldOrMethodInfo parseFieldOrMethodInfo() throws IOException {
		FieldOrMethodInfo result = new FieldOrMethodInfo(in.readUnsignedShort(), in.readUnsignedShort(),
	...
	public String toString() {
		...
		try {
			...
			for (int i = 1; i < constantPool.length; i++) {
				...
				if (entry.tag == CONSTANT_DOUBLE || entry.tag == CONSTANT_LONG)
					i++;
			}
