package a.entity.gus.y.convert1.stringtofont;
...
public class EntityImpl implements Entity, T {
	...
	public EntityImpl() throws Exception {
		availableNames = Outside.service(this, "gus.x.font.availablefontnames.list");
	}
	...
	private Font stringToFont(String s) throws Exception {
		...
		if (avail.contains(s))
			return new Font(s, DEFAULT_STYLE, DEFAULT_SIZE);
	...
	private int findStyle(String info) {
		if (info.toLowerCase().equals("plain"))
			return 0;
		if (info.toLowerCase().equals("bold"))
			return 1;
		if (info.toLowerCase().equals("italic"))
			return 2;
		if (info.toLowerCase().equals("bold|italic"))
			return 3;
		if (info.toLowerCase().equals("italic|bold"))
			return 3;
