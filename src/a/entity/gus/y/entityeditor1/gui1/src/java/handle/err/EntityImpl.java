package a.entity.gus.y.entityeditor1.gui1.src.java.handle.err;
...
public class EntityImpl implements Entity, P {
	...
	public void p(Object obj) throws Exception {
		...
		List xyzErrList = (List) ((R) data).r("xyzErrList");
		...
		if(fileName.equals("EntityImpl.java")) {
			if(xyzErrList!=null)
				for (int i = 0; i < xyzErrList.size(); i++) {
					Map m = (Map) xyzErrList.get(i);
					String entityName0 = (String) m.get(COL_ENTITY_NAME);
					if (entityName.equals(entityName0)) {
						String link = (String) m.get(COL_LINK);
						int pos = (int) m.get(COL_POS);
						high.addHighlight(pos, pos + link.length(), xyzErrPainter);
					}
				}
