package a.entity.gus.y.entitysys1.compile.entity;
...
public class EntityImpl implements Entity, P, F {
	...
	public EntityImpl() throws Exception {
		...
		findFrameworkPackageDir = Outside.service(this, "gus.x.framework.src.find.packagedir");
		...
		buildCmd = Outside.service(this, "gus.y.entitysys1.compile.entity.buildcmd");
		findJavac = Outside.service(this, "gus.y.jdk1.dir.jdkdirs.latest.javac");
		insertRow = Outside.service(this, "gus.y.entitydb1.entity_compile_err.insert");
		deleteRows = Outside.service(this, "gus.y.entitydb1.entity_compile_err.delete");
	...
	public boolean f(Object obj) throws Exception {
		...
		if (isCompiled(entityJavaFiles, binEntityPackageDir))
			return false;
		...
		for (File f : frameworkJavaFiles)
			p.println(dd(f));
		for (File f : entityJavaFiles)
			p.println(dd(f));
		p.close();
		...
		log("Compiling entity: " + entityName);
		...
		while ((line = br.readLine()) != null) {
			if (k == 0 && line.startsWith(packagePath)) {
				...
				errMap = new HashMap();
				errMap.put(COL_ENTITY_NAME, entityName);
				errMap.put(COL_FILE_NAME, fileName);
				errMap.put(COL_DATE, compileDate);
				errMap.put(COL_LINE_NB, lineNb);
				...
				errMap.put(COL_LINE_POS, pos);
				insertRow.p(new Object[] { cx, errMap });
				k = 0;
			}
		}
		int exitValue = process.exitValue();
		if(exitValue!=0) log("compiling exitValue: " + exitValue);
	...
	private boolean isCompiled(File[] entityJavaFiles, File binEntityPackageDir) {
		for (File entityJavaFile : entityJavaFiles) {
			...
			if (!entityClassFile.isFile())
				return false;
			if (entityClassFile.lastModified() < entityJavaFile.lastModified())
				return false;
		}
		return true;
	}
