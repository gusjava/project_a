package a.entity.gus.y.knowledgedb1.doc_y_extension.update;
...
public class EntityImpl implements Entity, P {
	...
	public void p(Object obj) throws Exception {
		...
		String sql = "UPDATE " + TABLE_NAME + " SET "
				+ COL_NAME + "=?, " + COL_KIND + "=?, " + COL_DIRECTION + "=?, "
				+ COL_ENTRY_POINT + "=?, " + COL_DESCRIPTION + "=?, " + COL_SPEC_ID + "=? "
				+ "WHERE " + COL_ID + "=?";

		executeUpdate(cx, sql,
				data.get(COL_NAME), data.get(COL_KIND), data.get(COL_DIRECTION),
				data.get(COL_ENTRY_POINT), data.get(COL_DESCRIPTION), data.get(COL_SPEC_ID),
				data.get(COL_ID));
	}
