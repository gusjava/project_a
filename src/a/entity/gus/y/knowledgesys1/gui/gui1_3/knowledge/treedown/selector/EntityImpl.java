package a.entity.gus.y.knowledgesys1.gui.gui1_3.knowledge.treedown.selector;
...
public class EntityImpl extends S1 implements Entity, G, I, V, ActionListener, TreeSelectionListener {
	...
	private List roots() {
		try {
			...
			List r = (List) ((R) engine).r("knowledgeLeafs");
	...
	private List children(Map node) {
		try {
			...
			for (int i = 0; i < links.size(); i++) {
				Long idLinker = toLong(((Map) links.get(i)).get("id_linker"));
				Map k = (Map) findKnowledge.t(new Object[] {cx, idLinker});
