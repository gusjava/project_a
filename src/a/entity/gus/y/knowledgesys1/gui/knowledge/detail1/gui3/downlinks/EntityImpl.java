...
public class EntityImpl implements Entity, I, P, V, ActionListener {
	...
	public EntityImpl() throws Exception {
		...
		addDownlink    = Outside.service(this, "*gus.y.knowledgesys1.gui.knowledge.downlink.add");
	...
	private void loaded() {
		try {
			...
			for (int i = 0; i < links.size(); i++) {
				...
				if (display.equals(m.get("linker")))
					model.addRow(new Object[]{m.get("linked"), m.get("type"), m.get("id_linker"), m.get("id_linked")});
			}
		} catch (Exception e) {
	...
	private void add_add() {
		if (data == null) return;
		try { addDownlink.p((String) data.get("code")); }
	...
	private void del_delete() {
		...
		String linkerCode = (String) data.get("code");
		String linkedCode = (String) model.getValueAt(modelRow, 0);
