package a.entity.gus.y.server1.engine.cmd.r.detailofobjective;
...
public class EntityImpl implements Entity, T {
	...
	{
		List list = (List) obj;
		if(list == null || list.isEmpty()) throw new Exception("r-detailofobjective: id manquant");
		String id = (String) list.get(0);
		Connection cx = (Connection) roadmapCx.g();
		Map result = new LinkedHashMap();

		Statement st = cx.createStatement();
		ResultSet rs = st.executeQuery("SELECT * FROM objective WHERE id = " + id);
		...
		ResultSet rs2 = st2.executeQuery("SELECT TAG FROM objective_tag WHERE ID_OBJECTIVE = " + id);
