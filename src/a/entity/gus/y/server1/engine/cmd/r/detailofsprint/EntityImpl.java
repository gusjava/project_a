package a.entity.gus.y.server1.engine.cmd.r.detailofsprint;
...
public class EntityImpl implements Entity, T {
	...
	{
		List list = (List) obj;
		if(list == null || list.isEmpty()) throw new Exception("r-detailofsprint: id manquant");
		String id = (String) list.get(0);
		Connection cx = (Connection) roadmapCx.g();
		Map result = new LinkedHashMap();

		Statement st = cx.createStatement();
		ResultSet rs = st.executeQuery("SELECT * FROM sprint WHERE id = " + id);
