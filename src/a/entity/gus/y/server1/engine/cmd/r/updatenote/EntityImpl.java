package a.entity.gus.y.server1.engine.cmd.r.updatenote;
...
public class EntityImpl implements Entity, T {
	...
	{
		if(!(obj instanceof Map)) throw new Exception("r-updatenote: JSON manquant (utiliser :<json>)");
		Map fields = new HashMap((Map) obj);
		Object id = fields.remove("id");
		if(id == null) throw new Exception("r-updatenote: champ 'id' manquant dans le JSON");
		Connection cx = (Connection) roadmapCx.g();
		return sqlUpdate.t(new Object[]{cx, buildUpdate("note", fields, id)});
	}

	private String buildUpdate(String table, Map fields, Object id) throws Exception
	{
		if(fields.isEmpty()) throw new Exception("r-updatenote: aucun champ \u00e0 mettre \u00e0 jour");
