package a.entity.gus.z.appli1.gui2_3_4.z.list;
...
//				entityDuplicate.p(new Object[] { engine1, getSelection(), table });
	...
	private class TableCellRenderer1 extends JLabel implements TableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
			...
			String zName = (String) table.getValueAt(row, 0);
			int errNumber = getErrorNumber(zName);
			...
			setForeground(getEntityForeground(zName));
			return this;
		}
	}
	...
	private void buildDataFull() throws Exception {
		...
		List zNameList = (List) engine.r("zNameList");
		Map entitiesByYName = (Map) engine.r("entitiesByZName");
		
		int nb = zNameList.size();
		for (int i = 0; i < nb; i++) {
			String zName = (String) zNameList.get(i);
			List entities = (List) entitiesByYName.get(zName);
			int entityNb = entities.size();
			dataFull.add(new String[] { zName, ""+entityNb });
		}
	}
...
//			refreshLabelNumberError();
	...
	private boolean isLocked(String zName)
	{
		try
		{
			Set lockSet = lockSet();
			return lockSet!=null && lockSet.contains(zName);
		}
		catch(Exception e)
