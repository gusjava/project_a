package a.entity.gus06.appli.gusexplorer.data.manager.op.remove;
...
public class EntityImpl implements Entity, T {
	...
	public Object t(Object obj) throws Exception
	{
		...
		if(data instanceof File) return perform_remove_file(list,(File) data);
		if(data instanceof List) return perform_remove_list(list,(List) data);
		
		throw new Exception("Invalid data type: "+data.getClass().getName());
	}
	
	
	private Object perform_remove_list(List l, List l1) throws Exception
	{
		if(l1.isEmpty()) return null;
		if(l1.size()==1)
		{
			File f = (File) l1.get(0);
			return perform_remove_file(l,f);
		}
		...
		for(int i=0;i<l1.size();i++)
		{
			...
			if(perform_remove_file(l,f)!=null) l1_.add(f);
		}
	...
	private Object perform_remove_file(List l, File f) throws Exception
	{
		f = file(f);
