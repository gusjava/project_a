package a.entity.gus06.file.runtask.dll.display.infos;

import a.framework.*;
import java.io.File;
import java.util.Set;

public class EntityImpl implements Entity, P {

	public String creationDate() {return "20191201";}


	private Service findType;
	private Service showMessage;

	public EntityImpl() throws Exception
	{
		findType = Outside.service(this,"gus06.env.windows.dll.findtype");
		showMessage = Outside.service(this,"gus06.swing.optionpane.showmessage.info");
	}

	
	public void p(Object obj) throws Exception
	{
		Object[] o = (Object[]) obj;
		if(o.length!=3) throw new Exception("Wrong data number: "+o.length);
		
		File file = (File) o[0];
		Object progress = o[1];
		Set interrupt = (Set) o[2];
		
		if(progress!=null) ((V)progress).v("size","1");
		
		String type = (String) findType.t(file);
		
		if(progress!=null) ((E)progress).e();
		
		String message = "DLL type: "+type;
		showMessage.p(new String[]{message,"DLL INFOS"});
	}
}
