 ...
 * </p>
 * <p>
 * Note that if the TrackType is Audio, you must add the AudioTrack member.
 * Likewise VideoTrack for Video.
 * </p>
 */
public class MatroskaFileTrack {
	...
	public enum TrackType {
		VIDEO(1), AUDIO(2), COMPLEX(3), LOGO(0x10), SUBTITLE(0x11), BUTTONS(0x12), CONTROL(0x20);
...
//    LOG.info("Track type set to {}", getTrackType().type);
		...
		// final UnsignedIntegerElement trackSeekPrerollElem = (UnsignedIntegerElement)
		// doc.createElement(MatroskaDocTypes.TrackSeekPreroll);
		...
		if (this.getTrackType() == TrackType.VIDEO) {
			...
			final FloatElement trackAudioOutputSamplingFrequencyElem = MatroskaDocTypes.OutputSamplingFrequency
					.getInstance();
