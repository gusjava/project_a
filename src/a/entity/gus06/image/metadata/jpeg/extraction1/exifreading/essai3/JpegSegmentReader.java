...
public class JpegSegmentReader
{
	 ...
	 */
	public JpegSegmentReader(File file) throws Exception
	{
		_file = file;
		_data = null;
		_stream = null;
	 ...
	 */
	public JpegSegmentReader(byte[] fileContents) throws Exception
	{
		_file = null;
		_data = fileContents;
		_stream = null;
	 ...
	 */
	public JpegSegmentReader(InputStream in) throws Exception
	{
		_stream = in;
		_file = null;
		_data = null;
	 ...
	 * @throws Exception for any problems processing the Jpeg data,
	 *		 including inner IOExceptions
	 */
	public byte[] readSegment(byte segmentMarker) throws Exception
	{
		...
		try {
			...
			do {
				...
				if ((segmentIdentifier & 0xFF) != 0xFF) {
					throw new Exception("expected jpeg segment start identifier 0xFF at offset " + offset + ", not 0x" + Integer.toHexString(segmentIdentifier & 0xFF));
				}
				offset++;
				...
				if (segmentLength > inStream.available())
					throw new Exception("segment size would extend beyond file stream length");
				else if (segmentLength < 0)
					throw new Exception("segment size would be less than zero");
			...
			//throw new Exception("IOException processing Jpeg file", ioe);
			throw new Exception("IOException processing Jpeg file: " + ioe.getMessage(), ioe);
		} finally {
			try {
				if (inStream != null) {
					inStream.close();
	 ...
	 * @throws Exception for any problems obtaining the stream
	 */
	private BufferedInputStream getJpegInputStream() throws Exception
	{
		if (_stream!=null) {
		...
		if (_data == null) {
			try {
				...
				throw new Exception("Jpeg file does not exist", e);
			}
		} else {
