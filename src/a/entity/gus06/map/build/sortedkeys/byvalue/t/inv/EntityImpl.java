package a.entity.gus06.map.build.sortedkeys.byvalue.t.inv;

import a.framework.*;
import java.util.Map;
import java.util.List;
import java.util.Collections;
import java.util.Comparator;
import java.util.ArrayList;

public class EntityImpl implements Entity, T {

	public String creationDate() {return "20220628";}


	private Service cache;
	
	public EntityImpl() throws Exception
	{
		cache = Outside.service(this,"gus06.feature.cache.t");
	}
	
	
	public Object t(Object obj) throws Exception
	{
		Object[] o = (Object[]) obj;
		if(o.length!=2) throw new Exception("Wrong data number: "+o.length);
		
		Map input = (Map) o[0];
		T trans = (T) cache.t(o[1]);
		
		List keys = new ArrayList(input.keySet());
		Collections.sort(keys,new Comparator1(input, trans));
		return keys;
	}
	
	
	private class Comparator1 implements Comparator
	{
		private Map map;
		private T t;
		
		public Comparator1(Map map, T t)
		{
			this.map = map;
			this.t = t;
		}
		
		public int compare(Object key1, Object key2)
		{
			Comparable r1 = (Comparable) trans(t,map.get(key1));
			Comparable r2 = (Comparable) trans(t,map.get(key2));
			
			return r2.compareTo(r1);
		}
	}
	
	
	
	private Object trans(T t, Object o)
	{
		try{return t.t(o);}
		catch(Exception e)
		{Outside.err(this,"trans(T,Object)",e);}
		return null;
	}
}