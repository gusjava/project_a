package a.entity.gus06.string.transform.block.keepline.all;
...
public class EntityImpl implements Entity, T {
	...
	public Object t(Object obj) throws Exception
	{
		...
		for(int i=0;i<nb;i++)
		{
			...
			for(int j=0;j<block.size();j++)
			{
				...
				if(inAllOtherBlock(blocks, j, line))
				{
					b.append(line);
					b.append(DELIM);
				}
	...
	private boolean inAllOtherBlock(List blocks, int j, String line)
	{
		int nb = blocks.size();
		for(int i=0;i<nb;i++) if(i!=j)
		{
			List block = (List) blocks.get(i);
			if(!block.contains(line)) return false;
		}
		return true;
	}
}