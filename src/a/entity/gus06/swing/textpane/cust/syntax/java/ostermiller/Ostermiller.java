...
public class Ostermiller {
	 ...
	 * Create a new Demo
	 */
	public Ostermiller(JTextPane textPane, Map styles) {
	...
	private class Colorer extends Thread {
		...
		public Colorer() {
			super("THREAD_"+Ostermiller.class.getName()+"_colorer");
		}
		/**
		...
		public void color(int position, int adjustment)
		{
			...
			{
				if(lastPosition < position - adjustment) change -= lastPosition - position;
...
class DocPositionComparator implements Comparator{
	...
	public int compare(Object o1, Object o2)
	{
		...
		if(o1 instanceof DocPosition) return -1;
		if(o2 instanceof DocPosition) return 1;
		if(o1.hashCode() < o2.hashCode()) return -1;
		if(o2.hashCode() > o1.hashCode()) return 1;
		return 0;
	}
}
...
class DocumentReader extends Reader {
	...
	public int read()
	{
		if(position >= document.getLength()) return -1;
		...
		catch(BadLocationException x){return -1;}
	}
	...
	public int read(char[] cbuf, int off, int len)
	{
		...
		try
		{
			...
			position += length;
			for(int i=0; i<length; i++) cbuf[off+i] = s.charAt(i);
			return length;
		}
		catch (BadLocationException x){return -1;}
	}
	...
	{
		if(n <= document.getLength()) position = n;
