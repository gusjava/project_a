...
public class EntityImpl implements Entity, P, V, R {
	...
	private void drawLabel(Graphics2D g2, String name)
	{
		...
		{
			g2.setPaint(BACKGROUND_PANEL);
			g2.fillRoundRect(label.x-MARGIN, label.y-MARGIN, label.width+2*MARGIN, label.height+2*MARGIN, EDGE, EDGE);
			g2.setPaint(background);
			g2.drawRoundRect(label.x-MARGIN, label.y-MARGIN, label.width+2*MARGIN, label.height+2*MARGIN, EDGE, EDGE);
		}
		else
		{
			g2.setPaint(background);
