package a.entity.gus06.sys.drawingpanel1.d.multi_cross2;
...
public class EntityImpl implements Entity, T {
	...
	private class Holder implements P, V
	{
		...
		public void p(Object obj) throws Exception
		{
			...
			for(int i=0;i<size;i++)
			{
				Point2D p1 = new Point2D.Double(pp[i].getX()-length,pp[i].getY()-length);
				Point2D p2 = new Point2D.Double(pp[i].getX()+length,pp[i].getY()+length);
				g2.draw(new Line2D.Double(p1,p2));
				
				p1 = new Point2D.Double(pp[i].getX()+length,pp[i].getY()-length);
				p2 = new Point2D.Double(pp[i].getX()-length,pp[i].getY()+length);
				g2.draw(new Line2D.Double(p1,p2));
			}
		}
	}
