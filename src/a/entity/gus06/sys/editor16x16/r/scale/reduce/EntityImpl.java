package a.entity.gus06.sys.editor16x16.r.scale.reduce;

import a.framework.*;
import java.awt.Color;
import java.awt.image.BufferedImage;
import javax.swing.plaf.OptionPaneUI;
import javax.swing.JOptionPane;
import javax.swing.border.TitledBorder;

public class EntityImpl implements Entity, T {

	public String creationDate() {return "20250828";}

	public final static String TRANSPARENT = "255-255-255-0";

	private Service resize;
	private Service dataToImage;

	public EntityImpl() throws Exception
	{
		resize = Outside.service(this,"gus06.awt.bufferedimage.resize.s16x16.reduced");
		dataToImage = Outside.service(this,"gus06.sys.editor16x16.t.datatoimage");
	}


	public Object t(Object obj) throws Exception
	{
		if(obj==null) return null;
		String[][] data = (String[][]) obj;
		int n = data.length;
		
		String[] values = new String[n];
		for(int i=0;i<n;i++) values[i] = ""+(i+1);
		
		Object res = JOptionPane.showInputDialog(null, 
			"Choose reduced size", 
			"Size chooser", 
			JOptionPane.PLAIN_MESSAGE, 
			null, values, values[0]);
			
		if(res==null) return data;
		
		int length = Integer.parseInt(""+res);
		
		BufferedImage image = (BufferedImage) dataToImage.t(data);
		image = (BufferedImage) resize.t(new Object[]{image, length});
		
		String[][] newData = new String[n][n];
		for(int i=0;i<n;i++)
		for(int j=0;j<n;j++)
		newData[i][j] = encodeFromImage(image, length, i, j);
		
		return newData;
	}
	
	
	private String encodeFromImage(BufferedImage image, int max, int x, int y)
	{
		if(x>=max || y>=max) return TRANSPARENT;
		
		int rgba = image.getRGB(y,x);
		
		int a = (rgba >> 24) & 0xFF;
		int r = (rgba >> 16) & 0xFF;
		int g = (rgba >> 8)  & 0xFF;
		int b = rgba         & 0xFF;
		
		if(a==0) return TRANSPARENT;
		return r+"-"+g+"-"+b+"-"+a;
	}
}