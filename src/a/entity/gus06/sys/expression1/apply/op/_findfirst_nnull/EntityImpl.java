package a.entity.gus06.sys.expression1.apply.op._findfirst_nnull;

import a.framework.*;
import java.util.Map;
import java.util.List;
import java.util.Set;

public class EntityImpl implements Entity, T {

	public String creationDate() {return "20231119";}


	private Service filter;
	private Service perform;
	
	public EntityImpl() throws Exception
	{
		filter = Outside.service(this,"gus06.data.filter.isnnull");
		perform = Outside.service(this,"gus06.data.perform.findfirst");
	}

	
	public Object t(Object obj) throws Exception
	{
		Object[] o = (Object[]) obj;
		if(o.length!=2) throw new Exception("Wrong data number: "+o.length);
		
		obj = o[0];
		
		if(obj==null) return null;
		
		if(obj instanceof List) return perform(obj);
		if(obj instanceof Object[]) return perform(obj);
		if(obj instanceof double[]) return perform(obj);
		if(obj instanceof int[]) return perform(obj);
		
		throw new Exception("Invalid data type: "+obj.getClass().getName());
	}
	

	public Object perform(Object obj) throws Exception
	{return perform.t(new Object[]{obj,filter});}
}