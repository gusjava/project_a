package a.entity.gus06.sys.expression1.apply.op._softmax;

import a.framework.*;

public class EntityImpl implements Entity, T {

	public String creationDate() {return "20171015";}


	private Service performIntArray;
	private Service performDoubleArray;
	
	public EntityImpl() throws Exception
	{
		performIntArray = Outside.service(this,"gus06.math.tabint.normalize.softmax");
		performDoubleArray = Outside.service(this,"gus06.math.tabdouble.normalize.softmax");
	}

	
	public Object t(Object obj) throws Exception
	{
		Object[] o = (Object[]) obj;
		if(o.length!=2) throw new Exception("Wrong data number: "+o.length);
		obj = o[0];
		
		if(obj==null) return null;
		
		if(obj instanceof int[]) return performIntArray.t(obj);
		if(obj instanceof double[]) return performDoubleArray.t(obj);
		
		throw new Exception("Invalid data type: "+obj.getClass().getName());
	}
}
