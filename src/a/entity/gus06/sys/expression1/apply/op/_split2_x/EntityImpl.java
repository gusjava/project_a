package a.entity.gus06.sys.expression1.apply.op._split2_x;

import a.framework.*;
import java.net.URL;

public class EntityImpl implements Entity, T {

	public String creationDate() {return "20180513";}


	private Service perform;


	public EntityImpl() throws Exception
	{
		perform = Outside.service(this,"gus06.data.perform.split2.first");
	}

	
	public Object t(Object obj) throws Exception
	{
		Object[] o = (Object[]) obj;
		if(o.length!=2) throw new Exception("Wrong data number: "+o.length);
		obj = o[0];
		
		if(obj==null) return null;
		
		if(obj instanceof String) return new T1(obj);
		if(obj instanceof URL) return new T1(obj.toString());
		
		throw new Exception("Invalid data type: "+obj.getClass().getName());
	}
	
	
	private class T1 implements T
	{
		private Object data;
		public T1(Object data) {this.data = data;}
		
		public Object t(Object obj) throws Exception
		{
			String cut = (String) obj;
			return new T2(data,cut);
		}
	}
	
	
	private class T2 implements T
	{
		private Object data;
		private String cut;
		
		public T2(Object data, String cut)
		{
			this.data = data;
			this.cut = cut;
		}
		
		public Object t(Object obj) throws Exception
		{
			return perform.t(new Object[]{data,cut,obj});
		}
	}
}
