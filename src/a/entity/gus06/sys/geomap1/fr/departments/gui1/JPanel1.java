...
public class JPanel1 extends JPanel implements MouseListener, MouseMotionListener {
	...
	public static final Color DEFAULT_COLOR_OUTSIDE = Color.LIGHT_GRAY;
	...
	private Color outsideColor = DEFAULT_COLOR_OUTSIDE;
	...
	*/
    ...
    private void walk(Node node) {
        ...
        if (node.getNodeType()==Node.ELEMENT_NODE && nb==0) {
            ...
            if(isDepartementElement(element)) {
...
//        g2d.setRenderingHint(RenderingHintsKeyExt.KEY_TRANSCODING, RenderingHintsKeyExt.VALUE_TRANSCODING_PRINTING);
    ...
    private int dx() {
    	return (int) ((getWidth() - bounds.width*scaleFactor) * 0.5);
    }
    
    private int dy() {
    	return (int) ((getHeight() - bounds.height*scaleFactor) * 0.5);
    }
    ...
    private String findCode(String id) {
    	...
    	if(id.startsWith("dep_")) return id.substring(4);
    	return null;
    }
    ...
    private String findName(String code) {
    	...
    	if(!UtilDepartment.DEPARTMENTS.containsKey(code)) return code+"?";
        return UtilDepartment.DEPARTMENTS.get(code);
    }
    
    private boolean isDepartementElement(Element element) {
	...
	*/
	...
	private String buildTooltipText(String code) {
		if(code==null) return "";
		if(tooltipTextT==null) return code+" - "+hoverName;
