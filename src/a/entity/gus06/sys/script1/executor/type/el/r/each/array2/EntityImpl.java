package a.entity.gus06.sys.script1.executor.type.el.r.each.array2;
...
public class EntityImpl implements Entity, P {
	...
	public void p(Object obj) throws Exception
	{
		...
		Object[][] struct = (Object[][]) main;
		
		String name_k = getIndexName(var);
		String name_v = getElementName(var);
		String name_k_ = name_k + "_";
		
		int nb1 = struct.length;
		int nb2 = nb1>0 ? struct[0].length : 0;
		int total = nb1*nb2;
		...
		{
			int k = i*nb2+j;
			if(offset==null || offset.intValue()<=k)
			if(index==null || index.intValue()==k)
			{
				Object value = struct[i][j];
				...
				m.put("last",Boolean.valueOf(k==total-1));
				m.put("even",Boolean.valueOf(k%2==0));
				m.put("odd",Boolean.valueOf(k%2==1));
				m.put("progress",(k+1)+"/"+total);
				m.put("value",value);
				
				pool1.put(name_k,Integer.valueOf(k));
