package a.entity.gus06.sys.treeparser2.engine;
...
public class EntityImpl implements Entity, T {
	...
	public Object t(Object obj) throws Exception
	{
		...
		handle(input,total,children,b,0,delim);
	...
	private void handle(List input, int total, List children, StringBuffer b, int level, char delim) throws Exception
	{
		while(!input.isEmpty())
		{
			String line = (String) input.get(0);
			int index = total-input.size();
			int level1 = findLevel(line,delim);
			...
			else if(level1==level+1)
			{
				...
				handle(input,total,children1,b1,level+1,delim);
	...
	private int findLevel(String line, char delim)
	{
		for(int i=0;i<line.length();i++)
		if(line.charAt(i)!=delim) return i;
		return line.length();
	}
