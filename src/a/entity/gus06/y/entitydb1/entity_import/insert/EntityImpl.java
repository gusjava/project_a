package a.entity.gus06.y.entitydb1.entity_import.insert;

import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.SQLException;
import a.framework.*;

public class EntityImpl implements Entity, P {

	public String creationDate() {return "20251215";}

	public static final String TABLE_NAME = "entity_import";

	public static final String COL_ENTITY_NAME = "entity_name";
	public static final String COL_ENTITY_IMPORT = "entity_import";
	public static final String COL_ENTITY_IMPORT_PACKAGE = "entity_import_package";
	public static final String COL_ENTITY_IMPORT_WILDCARD = "entity_import_wildcard";

	public void p(Object obj) throws Exception
	{
		Object[] o = (Object[]) obj;
		if (o.length != 3) throw new Exception("Wrong data number: " + o.length);

		Connection cx = (Connection) o[0];
		String entityName = (String) o[1];
		String entityImport = (String) o[2];
		
		Boolean wildcard = entityImport.endsWith(".*");
		String entityImportPackage = findPackage(entityImport);
		
		String sql = "INSERT INTO " + TABLE_NAME + " (" 
		+ COL_ENTITY_NAME + "," 
		+ COL_ENTITY_IMPORT + "," 
		+ COL_ENTITY_IMPORT_PACKAGE + "," 
		+ COL_ENTITY_IMPORT_WILDCARD + ") VALUES (?,?,?,?)";
		
		try
		{
			executeUpdate(cx, sql, entityName, entityImport, entityImportPackage, wildcard);
		}
		catch (SQLException e)
		{
			String message = "Failed to insert row with entityName=" + entityName + " and `import`=" + entityImport;
			throw new Exception(message, e);
		}
	}

	private void executeUpdate(Connection cx, String sql, Object... params) throws SQLException
	{
		PreparedStatement st = cx.prepareStatement(sql);
		for (int i = 0; i < params.length; i++) st.setObject(i + 1, params[i]);
		st.executeUpdate();
		st.close();
	}
	
	private String findPackage(String path)
	{
		int lastDot = path.lastIndexOf('.');
		return lastDot != -1 ? path.substring(0, lastDot) : "";
	}
}
