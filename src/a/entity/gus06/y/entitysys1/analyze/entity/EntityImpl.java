package a.entity.gus06.y.entitysys1.analyze.entity;
...
public class EntityImpl implements Entity, T {
	...
	public Object t(Object obj) throws Exception
	{
		...
		File srcDir = (File) o[1];
		Set setRoot = (Set) o[2];                                                                                                                                                                     
		Set ignore1 = (Set) o[3];                                                                                                                                                                     

		File javaFile = (File) nameToFile.t(new Object[] { srcDir, entityName });
		return extractDataFrom(javaFile, entityName, setRoot, ignore1);
	}



	private Map extractDataFrom(
		File javaFile, 
		String entityName, 
		Set setRoot, 
		Set ignore1) throws Exception
	{
		try
		{
			...
			if (!package1.equals("gus06.entity." + entityName))
				throw new Exception("Invalid package value: " + package1+" for entity "+entityName);
			
			Set links1 = buildLinks(links, setRoot, entityName);
			Set missingLinks1 = buildMissingLinks(setRoot, resources, services, entityName, ignore1);
	...
	private Set buildLinks(Set links, Set setRoot, String entityName)
	{
		...
		while(it.hasNext())
		{
			...
			String link = n[1];
			if(isValidLink(link, setRoot, entityName)) links1.add(linkInfo);
		}
		return links1;
	}
		...
		Set ignore1) throws Exception
	{
		...
		while(it.hasNext())
		{
			...
			if(isValidMissingLink(s0, setRoot, entityName, ignore1))
			{
				missingLinks.add(s);
			}
			else if(s0.contains("#"))
			{
				...
				isValidMissingLink(p2, setRoot, entityName, ignore1))
				{
	...
	private boolean isValidMissingLink(String link, Set setRoot, String entityName, Set ignore1) throws Exception
	{
		...
		if(ignore1.contains(link)) return false;
	...
	private boolean isValidProvider(String providerId) throws Exception
	{
		...
		if(providerId.equals("inside")) return false;
		if(providerId.equals("property")) return false;
		return true;
	}
