...
public class EntityImpl implements Entity, P {
	...
	{
		boolean hasTable = queryOne(cx, "SELECT name FROM sqlite_master WHERE type='table' AND name != '" + TABLENAME_INITIALIZED + "' LIMIT 1")!=null;
		if(!hasTable)
		{
			createAll(cx,init);
			return;
	...
	private void dropAll(Connection cx) throws Exception
	{
		...
		ResultSet rs = st.executeQuery("SELECT name FROM sqlite_master WHERE type='table' AND name != '" + TABLENAME_INITIALIZED + "'");
		while (rs.next())
		{
	...
	{
		((P) init).p(cx);
		String sql1 = "CREATE TABLE IF NOT EXISTS " + TABLENAME_INITIALIZED + " (date TEXT)";
		execute(cx, sql1);
	...
	private Date retrieveInitializedDate(Connection cx) throws Exception
	{
		...
		Object result = queryOne(cx, sql2);
		return result != null ? parseDate(result.toString()) : null;
	}
